classdef (ConstructOnLoad) FaradayEmfForCircularCircuits
    properties
        e0 = 8.85418782e-12
        c = 299792458
        mu0
    ...
    methods
        ...
        function emf = emfAtPoint(obj,...
            ...
            emf = -obj.mu0 / (4 * pi) * rDrvOuter * dot(cross(dIdtVec, rVec), [0, 0, 1]) / norm(rVec)^3;
        end
        ...
        function emf = faradayEmf(obj,...
            ...
            function emfMat = calcEmfFromMatrix(rMsr, msrCircuitAngle, drvCircuitAngle)
                ...
                for i = 1:rows
                    for j = 1:cols
                        emfMat(i,j) = rMsr(i,j) * emfAtPoint(obj, dIdt, rMsr(i,j), rDrvOuter, rMsrOuter, separation, vertSeparation, msrCircuitAngle(i,j), drvCircuitAngle(i,j));
                    end
                end
            end
